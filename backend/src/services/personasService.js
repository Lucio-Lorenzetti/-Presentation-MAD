...
async function normalizar(d, idExistente = null) {
  ...
  if (out.dni) {
    ...
    const dup = await db.prepare('SELECT id, nombre FROM personas WHERE dni = ? AND deleted_at IS NULL AND id != ?').get(out.dni, idExistente ?? -1);
...
async function listar({ tipo, listaNegra, q } = {}) {
  ...
  if (q) { where.push('(p.nombre ILIKE ? OR p.dni ILIKE ? OR p.cuit ILIKE ? OR p.email ILIKE ?)'); params.push(`%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`); }
  return db.prepare(`
    SELECT p.*,
    ...
    FROM personas p WHERE ${where.join(' AND ')} ORDER BY LOWER(p.nombre)
  `).all(...params);
}

// Ficha completa: datos + garantes + propiedades (propietario) o contratos (inquilino).
async function obtener(id) {
  const persona = await db.prepare('SELECT * FROM personas WHERE id = ? AND deleted_at IS NULL').get(id);
  if (!persona) return null;
  persona.garantes = await db.prepare('SELECT * FROM garantes WHERE persona_id = ? AND deleted_at IS NULL ORDER BY id').all(id);
  if (persona.tipo === 'PROPIETARIO') {
    persona.propiedades = await db.prepare(`
      ...
      FROM propiedades p WHERE p.propietario_id = ? AND p.deleted_at IS NULL ORDER BY LOWER(p.direccion)
    `).all(id);
  } else {
    const hoy = hoyISO();
    persona.contratos = await db.prepare(`
...
async function crear(d) {
  ...
  const info = await db.prepare(`INSERT INTO personas (${cols.join(', ')}) VALUES (${cols.map(() => '?').join(', ')})`)
    .run(...cols.map(c => v[c]));
  const id = Number(info.lastInsertRowid);
  for (const g of d.garantes || []) await agregarGarante(id, g);
  return obtener(id);
}

async function actualizar(id, d) {
  const a = await db.prepare('SELECT * FROM personas WHERE id = ? AND deleted_at IS NULL').get(id);
  if (!a) return null;
  ...
  const cols = Object.keys(v);
  if (cols.length) {
    await db.prepare(`UPDATE personas SET ${cols.map(c => `${c} = ?`).join(', ')} WHERE id = ?`).run(...cols.map(c => v[c]), id);
  }
  return obtener(id);
}
...
async function eliminar(id) {
  ...
  const conPropiedades = (await db.prepare('SELECT COUNT(*) AS n FROM propiedades WHERE propietario_id = ? AND deleted_at IS NULL').get(id)).n;
  if (conPropiedades > 0) {
    throw new ErrorValidacion(`No se puede eliminar: es propietario de ${conPropiedades} propiedad(es). Reasignalas primero.`, 409);
  }
  return (await db.prepare("UPDATE personas SET deleted_at = now() WHERE id = ? AND deleted_at IS NULL").run(id)).changes > 0;
}

async function agregarGarante(personaId, g) {
  requerir(g, ['nombre']);
  ...
  await db.prepare('INSERT INTO garantes (persona_id, nombre, dni, telefono, domicilio, tipo_garantia) VALUES (?, ?, ?, ?, ?, ?)')
    .run(personaId, g.nombre.trim(), dni, nn(g.telefono), nn(g.domicilio), nn(g.tipoGarantia));
}

async function eliminarGarante(personaId, garanteId) {
  return (await db.prepare("UPDATE garantes SET deleted_at = now() WHERE id = ? AND persona_id = ? AND deleted_at IS NULL")
