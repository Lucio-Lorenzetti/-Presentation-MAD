...
function listar({ estado, q } = {}) {
  ...
  if (q) { where.push('(i.nombre LIKE ? OR p.direccion LIKE ?)'); params.push(`%${q}%`, `%${q}%`); }
  return db.prepare(`${SELECT_CONTRATO} WHERE ${where.join(' AND ')} ORDER BY c.id DESC`).all(...params).map(decorar);
}

function obtener(id) {
  const c = db.prepare(`${SELECT_CONTRATO} WHERE c.id = ? AND c.deleted_at IS NULL`).get(id);
  if (!c) return null;
  const hoy = hoyISO();
  const cuotas = db.prepare('SELECT * FROM cuotas WHERE contrato_id = ? ORDER BY periodo').all(id).map(q => {
...
function crear(d) {
  ...
  const prop = db.prepare('SELECT * FROM propiedades WHERE id = ? AND deleted_at IS NULL').get(d.propiedadId);
  ...
  const inq = db.prepare('SELECT * FROM personas WHERE id = ? AND deleted_at IS NULL').get(d.inquilinoId);
  ...
  return transaccion(() => {
    ...
    const ins = db.prepare('INSERT INTO cuotas (contrato_id, periodo, vencimiento, monto) VALUES (?, ?, ?, ?)');
    for (let i = 0; i < n; i++) {
      const p = sumarMeses(d.fechaInicio.slice(0, 7) + '-01', i);
      ins.run(id, p.slice(0, 7), vencimientoDe(p, diaVto), d.montoInicial);
    }
    db.prepare("UPDATE propiedades SET estado = 'ALQUILADA' WHERE id = ?").run(d.propiedadId);
    return obtener(id);
  });
}
...
function cambiarEstado(id, estado) {
  ...
  transaccion(() => {
    db.prepare('UPDATE contratos SET estado = ? WHERE id = ?').run(estado, id);
    db.prepare("UPDATE propiedades SET estado = 'DISPONIBLE' WHERE id = ?").run(c.propiedad_id);
  });
  return obtener(id);
}
...
function aplicarAjuste(id, d = {}) {
  ...
  transaccion(() => {
    db.prepare('UPDATE contratos SET monto_actual = ?, proxima_actualizacion = ? WHERE id = ?').run(montoNuevo, proxima, id);
    db.prepare("UPDATE cuotas SET monto = ? WHERE contrato_id = ? AND estado = 'PENDIENTE' AND vencimiento >= ?")
...
function resumen() {
  ...
  const activos = db.prepare("SELECT COUNT(*) AS n FROM contratos WHERE estado = 'ACTIVO' AND deleted_at IS NULL").get().n;
  const porVencer = db.prepare(
    "SELECT fecha_fin FROM contratos WHERE estado = 'ACTIVO' AND deleted_at IS NULL"
  ).all().filter(r => diasEntre(hoy, r.fecha_fin) <= 60 && diasEntre(hoy, r.fecha_fin) >= 0).length;
  ...
  `).get(mes).t;
  const cobrado = db.prepare("SELECT COALESCE(SUM(total), 0) AS t FROM pagos WHERE substr(fecha, 1, 7) = ?").get(mes).t;
